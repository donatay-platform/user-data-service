...
- [ ] Убрать `allowedOriginPatterns("*")` вместе с credentials.
- [ ] Разрешить CORS только для нужных origin-ов через конфигурацию.
- [ ] Добавить отдельные настройки CORS для `local` и `prod`.
- [ ] Проверить protected endpoints без токена.
- [ ] Проверить public endpoints без токена.

## 1.4. JaCoCo и качество покрытия

- [ ] Убрать лишние исключения из JaCoCo.
- [ ] Оставить исключения только для generated DTO/API, bootstrap-класса и простых моделей при необходимости.
- [ ] Добавить проверку `BRANCH` coverage не ниже 70%.
- [ ] Добавить отчёт покрытия в стандартный lifecycle.
- [ ] Проверить, что тесты реально покрывают use case-и, security и web-слой.
...
- [ ] `1.3` Закрыть CORS.
- [ ] `1.4` Настроить честное покрытие JaCoCo.
