...
- [x] Появилось полноценное Vite + React + TypeScript frontend-приложение.
- [x] Добавлен TypeScript.
- [x] Добавлен сборщик Vite.
- [x] Код разделён на API-слой, компоненты приложения и стили.
...
- [x] Обновить frontend API URLs после разделения сервисов.
...
- [x] Заменить MFA-login по email на flow через `challengeId`.
- [x] Добавить сущность/таблицу для MFA challenge или временное хранилище с TTL.
- [x] Добавить TTL для MFA challenge.
- [x] Добавить ограничение числа попыток ввода MFA-кода.
- [x] Убрать возврат SMS-кода из HTTP-ответа.
- [x] В dev-профиле разрешить логировать факт отправки SMS-кода без значения кода.
- [x] В prod-профиле запретить логировать значение SMS-кода.

## 3.3. Rate limiting и аудит

- [x] Добавить ограничение попыток логина.
- [x] Добавить ограничение попыток MFA-login.
- [x] Добавить ограничение отправки SMS-кодов.
- [x] Добавить аудит важных security-событий MVP:
  - регистрация;
  - логин;
  - неудачный логин;
  - MFA required;
  ...
  - MFA verify;
  - отправка SMS-кода.
- [ ] Добавить аудит будущих security-событий после появления endpoints:
  - смена email;
  - смена пароля.
...
- [x] Настроить env-переменную для API URL.
- [ ] Подключить generated API client из OpenAPI.
- [x] Разбить код на API-слой, приложение и стили.

## 6.2. Auth UI

- [x] Экран регистрации.
- [x] Экран логина.
- [x] Экран MFA challenge.
- [ ] Безопасное хранение токена после оценки вариантов.
- [x] Обработка ошибок API понятным текстом.

## 6.3. Profile UI

- [x] Экран профиля пользователя/стримера для локальной проверки.
...
- [x] Tests для API success/error states.
...
- [x] `3.2` Переделать MFA flow.
- [ ] `4.x` Добавить домен донатов.
- [ ] `5.x` Добавить mock payments.
- [ ] `6.x` Довести frontend MVP: ESLint/Prettier, generated API client, storage strategy, public donation UI.
...
- [x] 2026-07-07 — MFA-login переведён на `challengeId`: добавлена таблица `auth_service.mfa_login_challenges`, TTL challenge, лимит попыток и проверка одноразового использования.
- [x] 2026-07-07 — в `auth-service` добавлен in-memory rate limiting для login, MFA-login и отправки SMS-кодов; при превышении лимита возвращается `429 Too Many Requests`.
- [x] 2026-07-07 — фронт подготовлен для локальных проверок: вместо одного HTML-прототипа создан Vite + React + TypeScript клиент с раздельными base URL для `auth-service` и `user-data-service`, auth/MFA challenge flow, profile read/update и базовыми API-тестами.
